...
CERT_BUNDLE="${SCRIPT_DIR}/server.pem"
CSR_FILE="${SCRIPT_DIR}/req.csr"


if [ ! -f $CERT_FILE ] && [ ! -f $KEY_FILE ] && [ ! -f $CERT_BUNDLE ]; 
    then
        echo "Certifcate Not Found: Generating Self Signed Certificate..."
        openssl req -nodes -newkey rsa:2048 -keyout $KEY_FILE -out $CSR_FILE -subj "/C=US/ST=Laziness/L=Lazytown/O=Lazy Nerds/OU=Department of Lazy/CN=localhost"
